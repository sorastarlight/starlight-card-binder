-- V88.2.1 Trade and notification routing hotfix - corrected
...
revoke all on function public.create_trade_offer(text, jsonb, jsonb, text)
from public, anon;

grant execute on function public.create_trade_offer(text, jsonb, jsonb, text)
to authenticated;

-- Normalize existing Daily Booster notification routes.
-- The correct table is public.user_notifications.
update public.user_notifications
set
  route = 'daily',
  route_params = '{}'::jsonb
where notification_type = 'daily_booster'
   or lower(coalesce(route, '')) in (
     'daily-booster',
     'daily-booster.html',
     'free-daily-booster',
     'binder.html?view=daily'
   );
