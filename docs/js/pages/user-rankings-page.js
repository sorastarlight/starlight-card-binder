...
export function initUserRankings(container = document) {
  if (!container) return null;

  const root = container.querySelector('#rankingsList');
  const meta = container.querySelector('#rankingsMeta');
  const status = container.querySelector('#rankingsStatus');
  const searchInput = container.querySelector('#rankingsSearch');
  const sortSelect = container.querySelector('#rankingsSort');
  const pager = container.querySelector('#rankingsPager');
  const liveRegion = container.querySelector('#rankingsLive');
  ...
  async function loadWishlistPanel(item) {
    ...
    container.querySelectorAll('.rankings-item.is-open').forEach(other => {
  ...
  return {
    refresh: () => loadRankings({ resetOffset: true }),
    destroy() {
