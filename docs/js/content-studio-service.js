...
export async function saveBooster(payload,slots=[]){const {data,error}=await supabase.rpc('admin_save_booster_complete_v9012',{payload,requested_slots:slots});if(error)throw error;return data;}
...
export async function safeDeleteBooster(id){const {data,error}=await supabase.rpc('admin_safe_delete_booster_v9012',{requested_id:id});if(error)throw error;return data;}
