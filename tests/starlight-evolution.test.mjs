...
test('Starlight Evolution migration and client wiring are present', async () => {
  ...
  assert.match(evoCss, /st-evo-enhance-stage|stEvoEnhanceBurst|st-evo-enhance-pile/);
  assert.match(css, /Border-only frames — no inner rainbow holo wash/);
  assert.match(evoPage, /st-evo-result-modal/);
  assert.match(evoPage, /st-evo-enhance-stage/);
  assert.match(evoPage, /st-evo-enhance-layers/);
  assert.match(evoPage, /st-evo-ready-grid/);
  assert.match(evoPage, /st-evo-tier-showcase/);
  assert.match(evoPage, /starlight-evolution-page\.js\?v=1\.9\.0/);
  assert.match(evoPage, /starlight-evolution\.css\?v=2\.0\.0/);
  ...
  assert.match(evoPageJs, /showEvolutionResult|playEnhanceSequence/);
  assert.match(evoPageJs, /buildEnhanceLayerMarkup|is-charge|is-burst/);
