...
test('Starlight Evolution migration and client wiring are present', async () => {
  ...
  assert.match(evoCss, /st-evo-flair-stage|stEvoStackDrop|st-evo-stack-pile/);
  ...
  assert.match(evoPage, /st-evo-stack-layers/);
  assert.match(evoPage, /st-evo-ready-grid/);
  assert.match(evoPage, /st-evo-tier-showcase/);
  assert.match(evoPage, /starlight-evolution-page\.js\?v=1\.7\.0/);
  assert.match(evoPage, /starlight-evolution\.css\?v=1\.8\.0/);
  ...
  assert.match(evoPageJs, /buildStackLayerMarkup|is-charging|is-evolved/);
